...
public class DialogoAgregarEmpleado extends JDialog implements ActionListener{
	...
	private JDateChooser fechaNacimientoPareja;
	...
	private ArrayList hijosP;
	private ArrayList referenciasPersonalesP;
	private ArrayList referenciasLaboralesP;
	private JDateChooser fechaNacimientoHijo;
	private JDateChooser fechaInicioContratoRefLaboral;
	private JDateChooser fechaFinContratoRefLaboral;
//	private JDateChooser dateChooserFechaCompra;
	...
	public DialogoAgregarEmpleado() {
		...
		btnCancelar.setBounds(451, 556, 89, 23);
...
//		dateChooserFechaCompra.setBounds(119, 128, 218, 20);
		...
		btnSubirFoto.setBounds(68, 206, 130, 23);
		...
		fechaNacimientoPareja = new JDateChooser();
		...
		scrollPane_1.setBounds(371, 11, 281, 231);
		panel_9.add(scrollPane_1);
		
		fechaInicioContratoRefLaboral = new JDateChooser();
		fechaInicioContratoRefLaboral.setBounds(154, 185, 179, 20);
		panel_9.add(fechaInicioContratoRefLaboral);
		
		fechaFinContratoRefLaboral = new JDateChooser();
		fechaFinContratoRefLaboral.setBounds(154, 229, 179, 20);
		panel_9.add(fechaFinContratoRefLaboral);
		
		JButton btnAgregarRefLaboral = new JButton("Agregar");
		btnAgregarRefLaboral.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			}
		});
		btnAgregarRefLaboral.setBounds(538, 254, 89, 23);
		btnAgregarRefLaboral.setActionCommand("Agregar Ref Laboral");
		btnAgregarRefLaboral.addActionListener(this);
		panel_9.add(btnAgregarRefLaboral);
		...
		scrollPane_2.setBounds(366, 11, 286, 126);
		panel_8.add(scrollPane_2);
		
		JButton btnAgregarRefPersonal = new JButton("Agregar");
		btnAgregarRefPersonal.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
			}
		});
		btnAgregarRefPersonal.setBounds(537, 151, 89, 23);
		btnAgregarRefPersonal.setActionCommand("Agregar Ref Personal");
		btnAgregarRefPersonal.addActionListener(this);
		panel_8.add(btnAgregarRefPersonal);
		
		hijosP = new ArrayList();
		referenciasLaboralesP = new ArrayList<>();
		referenciasLaboralesP = new ArrayList<>();
	}
...
System.out.println(command);
	...
	else if (command.equals("Agregar Ref Personal")){
		agregarRefPersonal();
	}
	else if (command.equals("Agregar Ref Laboral")){
		agregarRefLaboral();
	}
...
private void agregarEmpleado() {
	...
	String nombresP = nombreEmpleado.getText();
	String apellidosP = apellidosEmpleado.getText();
	String sexoP = (String) sexoEmpleado.getSelectedItem();
	int cedulaP = Integer.parseInt(cedulaEmpleado.getText());
	String estadoCivilP = (String) estadoCivilEmpleado.getSelectedItem();
	String direcciónP = direccionEmpleado.getText();
	Date fechaNacimientoP = fechaNacimientoEmpleado.getDate();
	int telefonoFijoP = Integer.parseInt(telefonoEmpleado.getText());
	int telefonoCelularP = Integer.parseInt(celularEmpleado.getText());
	String correoP = correoEmpleado.getText();
	
	String nombresParejaP = nombrePareja.getText();
	String apellidosParejaP = apellidosPareja.getText();
	int cedulaParejaP = Integer.parseInt(cedulaPareja.getText());
	Date fechaNacimientoParejaP = fechaNacimientoPareja.getDate();
	
	Date actual = new Date();
	int edadP = actual.getYear() - fechaNacimientoParejaP.getYear();
	
	String cargoP = cargoEmpleado.getText();
	Double salarioFijoP = Double.parseDouble(salarioFijo.getText());
	Double auxilioTransporteP = Double.parseDouble(auxilioTransporte.getText());
	String tipoContratoP = (String) tipoContrato.getSelectedItem();
	String duracionContratoP = (String) duracionContrato.getSelectedItem();
	Date fechaInicioContratoP = fechaInicioContrato.getDate();
	Date fechaFinContratoP = fechaFinContrato.getDate();
	
	
	String pensionesP = null;
	String saludP = null;
	String arlP = null;
	Date fechaAfiliacionSSP = null;
	String solidaridadP = null;
	
	//TODO validar campos
	
	interfaz.agregarEmpleado(nombresP, apellidosP, sexoP, cedulaP, estadoCivilP, direcciónP, 
	...
	private void agregarHijo() {
		...
		int documentoP = Integer.parseInt(cedulaHijo.getText());
		Date fechaNacimientoP = fechaNacimientoHijo.getDate();
		
		Date actual = new Date();
		int edadP = actual.getYear() - fechaNacimientoP.getYear(); 
		
		if (nombreP != null && apellidosP != null && documentoP >= 0 && fechaNacimientoP != null && edadP >= 0){
			
			Hijo nuevo = new Hijo(nombreP, apellidosP, documentoP, fechaNacimientoP, edadP);
			hijosP.add(nuevo);	
		}
		else{
	...
	private void agregarRefPersonal() {
		
		String tipoP = Referencia.PERSONAL;
		String nombreP = nombreRefPersonal.getText();
		int telefonoP = -1;

		try{
			telefonoP = Integer.parseInt(telefonoRefPersonal.getText());
		}
		catch (Exception e){
			JOptionPane.showMessageDialog(this, "Debe ingresar un número válido en el campo de telefono", "Error", JOptionPane.INFORMATION_MESSAGE);
		}
		...
		if (tipoP != null && nombreP != null && telefonoP > 0 && conceptoP != null){
			
			if (empresaP == null || fechaInicioP == null || fechaFinP == null){
				...
				if (rta == JOptionPane.YES_OPTION){
					Referencia nueva = new Referencia(tipoP, nombreP, telefonoP, empresaP, conceptoP, fechaInicioP, fechaFinP);
					referenciasPersonalesP.add(nueva);
				}
			}
			else{
				Referencia nueva = new Referencia(tipoP, nombreP, telefonoP, empresaP, conceptoP, fechaInicioP, fechaFinP);
	...
	private void agregarRefLaboral() {
		
		String tipoP = Referencia.LABORAL;
		String nombreP = nombreRefPersonal.getText();
		int telefonoP = Integer.parseInt(telefonoRefPersonal.getText());
		String empresaP = empresaRefPersonal.getText();
		String conceptoP = conceptoRefPersonal.getText();
		Date fechaInicioP = fechaInicioContrato.getDate();
		Date fechaFinP = fechaFinContrato.getDate();
		...
		if (tipoP != null && nombreP != null && telefonoP > 0 && conceptoP != null){
			...
			if (empresaP == null || fechaInicioP == null || fechaFinP == null){
				...
				if (rta == JOptionPane.YES_OPTION){
					Referencia nueva = new Referencia(tipoP, nombreP, telefonoP, empresaP, conceptoP, fechaInicioP, fechaFinP);
					referenciasLaboralesP.add(nueva);
				}
			}
			else{
				Referencia nueva = new Referencia(tipoP, nombreP, telefonoP, empresaP, conceptoP, fechaInicioP, fechaFinP);
